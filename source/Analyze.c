...
//      һ
        ...
        if(StopFlag == 1)
        {
            u8 data_sample = AnalyzeBuf[DateCnt * 3 + 0];
            if(AnalyzeBuf[DateCnt * 3 + 1] & 0x01)                                 // ��ʼλ
            {
                 if(gInputSubParam[IN_I2C][0]) data_sample &= 0xFE; // R/W
                 if(gInputSubParam[IN_I2C][2]) data_sample >>= 1; // R/W MLO Hack to set address to 7bit
            }
            switch(Data_Format)                                                    // ������ʾ����
            {
            case ASCII:
                Char2Str(Str, data_sample);
                break;
            case DEX:
                u8ToDec3Str(Str, data_sample);
                break;
            case HEX:
                Char2HexFullStr(Str, data_sample);
                break;
            default:
                break;
            }

...
* Description  : Main process / capture process
* Param        : void 
*******************************************************************************/
void Process(void)
{
  ...
  gTimeBase = gItemParam[TIMEBASE];                   // Get current timebase sampling period
  memset(gSamplBuf, 0, RECORD_DEPTH * CNTx4K);        // clear sample buffer
  TriConfig();                                        // apply trigger sampling settings to the FPGA
  
  FPGA_ReadWrite(SMPL_CLR, 1, 0);                     // clear sampling memory in fpga
  Delay_mS(2);
  gFullmS = 1000;                                     //  
  //
  //  Delay_mS(500);
  ...
  if (SamplFlag == 1)
  {
    ...
    for (i = 0; i < RECORD_DEPTH; /*i++*/)
    {
      ...
      Buf[0] = (u8)(SamplData);                   // 
	  Buf[1] = (u8)(SamplData >> 8);              //
      Buf[2] = (u8)(SamplData >> 16);             //
      Buf[3] = (u8)(SamplData >> 24);             // 4bitͨ
      
      //          memcpy(&gSamplBuf[CNTx4K * i], Buf, CNTx4K);// �����ɼ��浽������
      //          i++;
      
      if ((Buf[0] & (FPGA_EMPTY | FPGA_FULL)) == (FPGA_EMPTY | FPGA_FULL)) //Full��Empty������
      {
        //              ���Բ鿴����ʹ�ã�ʵ��Full��Emptyʱ������Ч
    ...
    AnalyzeFrame(gItemParam[INTYPE]);                   // Э�����ݿ�ܷ���
  }
  
  gTransTimeSum = CalculateTimeSum(RECORD_DEPTH);          // ����������������ʱ���ܺ�
  ProcessSmallWins();
  ZC_Scale();                                              // ͼ����ʾ
}
...
//  s32 L_Cnt = 0;
    ...
    memset(gLCD_Buf, 0, LCD_BUF_WIDTH); 				  // Clear buffer
...
//  
    ...
    memcpy(gLCD_Backup, gLCD_Buf, LCD_BUF_WIDTH); 	//  copy buffer
    ShowWaveToLCD();                                                       // LCD
...
void ShowWaveToLCD(void)
{
    ...
    __Bios(DISPDEV, FPGA_MOD);                             // FPGA
    FPGA_ReadWrite(WFRM_DRAW, 0, 0);                       // 
    do
    {
        Info = FPGA_ReadWrite(FPGA_INFO, 0, 0) >> 8;       // �� FPGA ״̬
    } while (Info & BUSY);                                  // ���� DRAW ״̬ʱ�˳�
...
//  T1
    ...
    TransTimeSum = 0;
//  
    for (i = Mark1Cnt; i < RECORD_DEPTH; i++)                    // T1-T2
    {
 ...
 * Description  : �ɱ��IO��������
 * Param        : u8 Type EXT_SPI/EXT_UART/EXT_I2C
*******************************************************************************/
 ...
 * Param        : void 
 *
 * Note: Modified by Ralim to provide the following keyboard shortcuts:
 * 
